...
abstract public class LoadCommon {
  ...
  static /* package */ enum Inquiry {
    DOWNLOADDIR("downloaddir"), UPLOADDIR("uploaddir"), USERNAME("username");
    ...
    static public Inquiry parse(String key) {
      ...
      for (Inquiry x : Inquiry.values()) {
        if (key.equalsIgnoreCase(x.toString()))
          return x;
      }
      return null;
    }
  ...
  static /* package */ enum FileFormat {
    ...
    static public FileFormat getformat(String fmt) {
      ...
      for (FileFormat rf : FileFormat.values()) {
        if (fmt.equalsIgnoreCase(rf.toString()))
          return rf;
      }
      return null;
    }
  ...
  static public List<String> parseList(String params, char sep, boolean decode) {
  ...
  protected void sendReply(int code, String msg) {
    try {
      ...
      res.setContentType(usetext ? ContentType.text.getContentHeader() : ContentType.html.getContentHeader());
      try {
  ...
  static protected void reportRequest(HttpServletRequest req) {
