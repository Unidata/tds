...
public class RadarServerController implements InitializingBean {
  ...
  @ResponseBody
  public StationList stations(@PathVariable String dataset, final HttpServletRequest request) {
    if (!enabled)
      return null;
  ...
  HttpEntity<byte[]> handleQuery(@PathVariable String dataset,
      ...
      @RequestParam(value = "var", required = false) String[] vars, final HttpServletRequest request)
    ...
    try {
      if (!setTimeLimits(q, time, start, end, period, temporal))
        throw new UnsupportedOperationException(
            "Either a single time " + "argument, temporal=all, or a combination of time_start, "
    ...
    if (stations == null) {
      try {
        stations = getStations(di.getStationList(), lon, lat, north, south, east, west);
        ...
        throw new UnsupportedOperationException("Either a list of " + "stations, a lat/lon point, or a box defined by "
            + "north, south, east, and west parameters must be " + "provided.");
      }
    } else {
  ...
  boolean setTimeLimits(RadarDataInventory.Query query, String timePoint, String start, String end, String period,
      String temporal) throws ParseException {
    ...
    if (timeStart != null) {
      ...
      query.addDateRange(
          new CalendarDateRange(timeEnd.add((long) -duration.getValueInSeconds(), CalendarPeriod.Field.Second),
