...
public class GetCoverage extends WcsRequest {
  ...
  public GetCoverage(Request.Operation operation, String version, WcsDataset dataset, String coverageId, String crs,
      String responseCRS, Request.BoundingBox bbox, CalendarDateRange timeRange,
      WcsCoverage.VerticalRange verticalRange, Request.Format format) throws WcsException {
    ...
    if (!this.getWcsDataset().isAvailableCoverageName(coverageId))
      throw new WcsException(WcsException.Code.InvalidParameterValue, "coverage",
          "Unknown coverage identifier [" + coverageId + "].");
    ...
    if (!crs.equalsIgnoreCase(this.coverage.getDefaultRequestCrs()))
      throw new WcsException(WcsException.Code.InvalidParameterValue, "CRS",
          "Request CRS [" + crs + "] not allowed [" + this.coverage.getDefaultRequestCrs() + "].");
    ...
    if (responseCRS == null) {
      ...
      throw new WcsException(WcsException.Code.InvalidParameterValue, "response_CRS",
          "Response CRS [" + responseCRS + "] not the supported CRS [" + this.coverage.getNativeCrs() + "].");
  ...
  private LatLonRect convertBoundingBox(Request.BoundingBox bbox, CoverageCoordSys gcs) throws WcsException {
    ...
    // {
    // log.error( "convertBoundingBox(): BBOX [" + bbox + "] not contained in coverage BBOX ["+
    // covLatLonRect.toString2()+"].");
    // throw new WcsException( WcsException.Code.InvalidParameterValue, "BBOX", "BBOX [" + bbox + "] not contained in
    // coverage.");
    // }
