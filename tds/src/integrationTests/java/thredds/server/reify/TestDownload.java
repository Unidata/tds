...
public class TestDownload extends TestReify {
  ...
  static class TestCase extends AbstractTestCase {
    ...
    public String toString() {
      ...
      for (Map.Entry<String, String> entry : this.params.entrySet()) {
        buf.append(String.format("%s%s=%s", first ? "?" : "&", entry.getKey(), entry.getValue()));
      }
      return buf.toString();
    }
    ...
    public String makeURL() {
      ...
      pathBuilder.append("/?");
      String params = mapToString(this.params, true, "request", "format", "target", "url", "testinfo");
  ...
  @Override
  public void doOneTest(AbstractTestCase tc) throws Exception {
  ...
  protected void defineAllTestCases() {
    alltestcases
        .add(new TestCase("download", "http://host:80/thredds/fileServer/localContent/testData.nc", "nc3/testData.nc3", // baseline
            "format=netcdf3"));
    alltestcases.add(new TestCase("download", "http://host:80/thredds/fileServer/localContent/testData.nc",
        "nc4/testData.nc4", "format=netcdf4"));
    alltestcases.add(new TestCase("download", "http://host:80/thredds/dodsC/localContent/testData.nc", "testData.nc3", // baseline
        "format=netcdf3"));
    alltestcases.add(new TestCase("download", "http://host:80/thredds/dodsC/localContent/testData.nc", "testData.nc4",
        "format=netcdf4"));
  }
