...
abstract public class TestReify extends TdsUnitTestCommon {
  ...
  static enum Command {
    ...
    static public Command parse(String cmd) {
      ...
      for (Command x : Command.values()) {
        if (cmd.equalsIgnoreCase(x.toString()))
          return x;
      }
      return null;
    }
  ...
  public void getServerProperties(String server) throws Exception {
  ...
  public int callserver(HTTPMethod method) throws IOException {
    int code = 0;
    // Make method call
    method.execute();
    if (DEBUGPARTS) {
  ...
  static public String replyCompare(Map<String, String> result, Map<String, String> base) {
    StringBuilder b = new StringBuilder();
    // do two ways to catch added plus new
    for (Map.Entry<String, String> entry : result.entrySet()) {
      String basevalue = base.get(entry.getKey());
      if (basevalue == null) {
        b.append(String.format("Added: %s%n", entry.getKey()));
      } else {
        String rvalue = entry.getValue();
        if (!rvalue.equals(basevalue)) {
          b.append(String.format("Change: %s: %s to %s%n", entry.getKey(), basevalue, rvalue));
        }
      }
    }
    for (Map.Entry<String, String> entry : base.entrySet()) {
      ...
      if (rvalue == null) {
        b.append(String.format("Deleted: %s%n", entry.getKey()));
      }
    }
   ...
   */
  static public boolean deleteTree(String root, boolean deleteroot) {
    if (root == null || root.length() == 0)
      return false;
    File rootfile = new File(root);
    if (!rootfile.exists())
      return false;
    if (!deleteTree(rootfile))
      return false;
    if (deleteroot && !rootfile.delete())
      return false;
    return true;
  }

  static protected boolean deleteTree(File root) {
  ...
  static void filereport(String path) {
  ...
  static public Map<String, String> parseMap(String params, char sep, boolean decode) {
  ...
  static public String mapToString(Map<String, String> map, boolean encode, String... order) {
  ...
  static List<org.apache.http.Header> iter2list(Iterator<Object> e) {
