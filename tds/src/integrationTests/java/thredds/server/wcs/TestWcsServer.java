...
public class TestWcsServer {
  ...
  public void testGetCapabilites() throws IOException, JDOMException {
    ...
    // XPathFactory.instance().compile("ns:/WCS_Capabilities/ContentMetadata/CoverageOfferingBrief", Filters.element(),
    // null, NS_WCS);
    XPathExpression<Element> xpath =
        XPathFactory.instance().compile("//wcs:CoverageOfferingBrief", Filters.element(), null, NS_WCS);
  ...
  @Test
  public void testGetCoverageFuzzyTime() throws IOException { // no longer fails because we use closest time. LOOK is
                                                              // that ok?
  ...
  @Test
  public void testGetCoverageFailBadCoverageName() throws IOException { // should fail because coverage name doesnt
                                                                        // exist
    String endpoint = TestOnLocalServer.withHttpPath(
  ...
  public void saveGetCoverageNetcdf() throws IOException {
    String endpoint = TestOnLocalServer.withHttpPath(
        dataset2 + "&request=GetCoverage&COVERAGE=sst&BBOX=10,0.01,299.99,80&TIME=2002-12-01T00:00:00Z&FORMAT=NetCDF3");
  ...
  public void saveGetCoverageNetcdf2() throws IOException {
    String endpoint =
        TestOnLocalServer.withHttpPath(dataset1 + "&request=GetCoverage&COVERAGE=Temperature&FORMAT=NetCDF3");
  ...
  public void multipleVertGeotiffFail() throws IOException {
    String endpoint =
        TestOnLocalServer.withHttpPath(dataset1 + "&request=GetCoverage&COVERAGE=Temperature&FORMAT=Geotiff");
  ...
  private void showGetCoverage(String url, String grid, String time, String vert, String bb, String format,
      boolean showOnly) throws IOException {
