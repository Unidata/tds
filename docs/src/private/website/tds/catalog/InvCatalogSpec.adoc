...
of granularity appropriate for presentation to a user. A dataset is a
_<<directDataset>>_ if it contains at least one
link:#Dataset_Access_Methods[dataset access method], otherwise it is
a container for nested datasets, called a _<<collectionDataset>>_.
...
parent dataset. (You may also use an _authority_ element rather than an attribute.)
If a dataset has an _ID_ and an _authority_ attribute, then the combination of the two should be
...
If the _harvest_ attribute is true, then
...
entries, and is typically placed on _collection_ datasets.
...
appropriate metadata), then wherever else it should appear, make a
...
The _dataset_ element’s _serviceName_ (which can also be specified as a _serviceName_ element)
specifies which service to use for this dataset. As of 5.0, it is optional as long as you
specify the _dataType_ or _featureType_ of the dataset.
The urlPath attribute, in combination with the applicable service, is used to specify
...
more than one nested <<access>> elements, or use a <<compoundService>>.
...
The common case is that the access element is _implicit_, based on the dataset's _serviceName_ and _urlPath_.
...
<<dataType>> which are intended to map to the scientific data types from
the https://www.unidata.ucar.edu/software/netcdf/CDM/[Common Data Model
...
Note that if a _documentation_ element in a server-side catalog contains XHTML,
that XHTML will not be preserved when a client catalog is generated from it.
We currently have an https://github.com/Unidata/thredds/issues/1088[issue on GitHub]
to track the bug. This also means that the XHTML won't be displayed in the
HTML version of the client catalog (i.e. the dataset summary page) that TDS displays.
...
3.  a valid https://www.unidata.ucar.edu/packages/udunits/[udunits] date string
...
a valid https://www.unidata.ucar.edu/packages/udunits/[udunits] time
duration string.
...
https://www.unidata.ucar.edu/packages/netcdf-java/[Java-NetCDF library]
...
handled internally by THREDDS. A <<compoundService>> indicates that the service is composed of other services.
...
(*) Multiple access methods are defined whenever the service element is a <<compoundService>>. +

[[defaultService]]
=== Dataset Default Service

. A dataset may have a *_serviceName_* attribute or element directly in it.
. A dataset may _inherit_ a *_serviceName_* from a parent dataset.
. A dataset may contain an access element that defines a  *_serviceName_* and *_urlPath_*.
. A dataset may have a *_featureType_* or *_dataType_* defined and a *_urlPath_*, in which case it will use the default services for that type.

==== Examples

1. A _dataset_ element has a _urlPath_ attribute and inherits a _serviceName_ from a parent/ancestor dataset.
This is a common case as many catalogs will contain datasets that all refer to te same service.
+
[source,xml]
----
...
4. A _dataset_ may have a _featureType_ or <<dataType>> defined and a _urlPath_. Example:
+
[source,xml]
----
...
The <<../reference/services/StandardServices#,standard services>> for Grids will be used.
...
service base is _/airtemp/_, the resolved URL is _\http://thredds.ucar.edu/thredds/airtemp/_.
...
_FTP_, _DODS_, _WMS_, etc.) that allows a THREDDS-enabled
...
nested datasets, and there are no constraints on how the datasets are related.
...
_collectionType_ attribute that describes the relationship of its nested datasets.
...
image:../thread.png[THREDDS]This document was last updated March 2016.
