...
public class TestUpload extends TestReify {
  ...
  public void doOneTest(AbstractTestCase tc) throws Exception {
    ...
    if (prop_diff) {
      ...
      Assert.assertTrue(String.format("***Fail: Upload file (%d bytes) and Original file (%d bytes) differ in size",
          targetbytes.length, srcbytes.length), targetbytes.length == srcbytes.length);
  ...
  protected void defineAllTestCases() {
    ...
    try {
      ...
      /* ignore */}
    Assert.assertTrue("Cannot write testfile: " + testfile.getAbsolutePath(), testfile.canWrite());
    ...
    try {
      ...
      fw.write(text);
      fw.close();
      alltestcases.add(new TestCase(/* file= */testfile.getAbsolutePath(), true, /* target= */"target.txt"));
    } catch (IOException ioe) {
      Assert.assertTrue(
          String.format("Cannot write testfile: %s err=%s%n", testfile.getAbsolutePath(), ioe.getMessage()), false);
    }


  }

  protected org.apache.http.HttpEntity buildPostContent(TestCase tc) throws IOException {
