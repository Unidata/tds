...
public class TestSSH extends UnitTestCommon {
  ...
  static final String[] sshurls = {"https://" + SERVER + "/thredds/dodsC/localContent/testData.nc.dds"};
  ...
  static public void report(Result result, Integer counter) {
    System.err.printf("Result: code=%d content?=%b provider-calls=%d%n", result.status, result.contents.length,
        counter);
    System.err.flush();
  }
  ...
  static class TestProvider implements CredentialsProvider, Serializable {
    ...
    public Credentials getCredentials(AuthScope scope) // AuthScheme authscheme, String host, int port, boolean isproxy)
    {
      ...
      System.err.printf("TestCredentials.getCredentials called: creds=|%s| host=%s port=%d%n", creds.toString(),
  ...
  public void testSSH() throws Exception {
    ...
    Assume.assumeTrue("Version must be 1.8 (temporary), not: " + version, version.startsWith("1.8"));
   ...
   */
  @Test
  public void testMutualSSH() throws Exception {
    String version = System.getProperty("java.version");
    Assume.assumeTrue("Version must be 1.8 (temporary), not: " + version, version.startsWith("1.8"));
