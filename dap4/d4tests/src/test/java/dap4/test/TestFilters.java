...
public class TestFilters extends DapTestCommon {
  ...
  static class ConstraintTest {
    ...
    ConstraintTest(int id, String dataset, String ce, Dump.Commands template) {
      this(id, dataset, 0, ce, template, false);
    }

    ConstraintTest(int id, String dataset, int rows, String ce, Dump.Commands template) {
      this(id, dataset, rows, ce, template, false);
    }

    ConstraintTest(int id, String dataset, int rows, String ce, Dump.Commands template, boolean xfail) {
      if (alltests[id] != null)
  ...
  protected void defineAllTestcases(String root) {
    ConstraintTest.root = root;
    this.alltestcases.add(new ConstraintTest(1, "test_sequence_1.syn", "/s", new Dump.Commands() {
    ...
    this.alltestcases.add(new ConstraintTest(2, "test_sequence_1.syn", "/s|i1<0", new Dump.Commands() {
      public void run(Dump printer) throws IOException {
        int count = printer.printcount();
        for (int j = 0; j < count; j++) {
          printer.printvalue('S', 4);
  ...
  //////////////////////////////////////////////////
  // Primary test method
  boolean doOneTest(ConstraintTest testcase) throws Exception {
    boolean pass = true;
  ...
  boolean dodata(ConstraintTest testcase) throws Exception {
    boolean pass = true;
    String baseline;
  ...
  List<ConstraintTest> locate(Object pattern) {
