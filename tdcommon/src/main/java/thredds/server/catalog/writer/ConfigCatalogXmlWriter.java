...
public class ConfigCatalogXmlWriter extends CatalogXmlWriter {
   ...
   * curSelectorElem.setAttribute("lastModLimitInMillis", Long.toString(((LastModifiedLimitFilter)
   * curFilter).getLastModifiedLimitInMillis()));
   * curSelectorElem.setAttribute("atomic", curSelector.isApplyToAtomicDataset() ? "true" : "false");
   * curSelectorElem.setAttribute("collection", curSelector.isApplyToCollectionDataset() ? "true" : "false");
   * } else
   ...
   * else
   * userDefElem.addContent(new Comment("This class <" + className +
   * "> not yet supported. This XML is missing configuration information (of type " + configObj.getClass().getName() +
   * ")."));
   * }
   * 
   * return userDefElem;
