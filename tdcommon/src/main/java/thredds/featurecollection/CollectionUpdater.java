...
public class CollectionUpdater {
  ...
  public void scheduleTasks(FeatureCollectionConfig config, Logger logger) {
    ...
    JobDetail updateJob = JobBuilder.newJob(UpdateCollectionJob.class).withIdentity(collectionName, "UpdateCollection")
        .storeDurably().usingJobData(map).build();

    try {
      if (!scheduler.checkExists(updateJob.getKey())) {
        scheduler.addJob(updateJob, false);
      } else {
        logger.warn("scheduler failed to add updateJob for " + updateJob.getKey()
            + ". Another Job exists with that identification.");
      }
    } catch (Throwable e) {
    ...
    if (updateConfig.startupType != CollectionUpdateType.never) {
      ...
      SimpleTrigger startupTrigger = (SimpleTrigger) TriggerBuilder.newTrigger().withIdentity(collectionName, "startup")
          .startAt(runTime).forJob(updateJob).usingJobData(map).build();

      try {
        scheduler.scheduleJob(startupTrigger);
        logger.info("scheduleJob startup scan force={} for '{}' at {}", updateConfig.startupType.toString(),
            config.collectionName, runTime);
      } catch (Throwable e) {
    ...
    if (updateConfig.rescan != null) {
      ...
      CronTrigger rescanTrigger = TriggerBuilder.newTrigger().withIdentity(collectionName, "rescan")
          .withSchedule(CronScheduleBuilder.cronSchedule(updateConfig.rescan)).forJob(updateJob).usingJobData(map)
          .build();
     ...
     * if (logger != null)logger.info("Schedule proto update for '{}' cronExpr={}", config.collectionName,
     * pconfig.change);
     * 
     * } catch (Throwable e) {
